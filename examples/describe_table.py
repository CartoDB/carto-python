...
for i in all_datasets:
    ...
    if norm > 0 and norm <= 10:
        print '{:|<10}'.format(str(i.table.name)+'\t') + \
            '\t ' + str(round(val, 2)) + ' MB'
    elif norm > 10 and norm <= 20:
        print '{:|<20}'.format(str(i.table.name)+'\t') + \
            '\t ' + str(round(val, 2)) + ' MB'
    elif norm > 20 and norm <= 30:
        print '{:|<30}'.format(str(i.table.name)+'\t') + \
            '\t ' + str(round(val, 2)) + ' MB'
    elif norm > 30 and norm <= 40:
        print '{:|<40}'.format(str(i.table.name)+'\t') + \
            '\t ' + str(round(val, 2)) + ' MB'
    elif norm > 40 and norm <= 50:
        print '{:|<50}'.format(str(i.table.name)+'\t') + \
            '\t ' + str(round(val, 2)) + ' MB'
    elif norm > 50 and norm <= 60:
        print '{:|<60}'.format(str(i.table.name)+'\t') + \
            '\t ' + str(round(val, 2)) + ' MB'
    elif norm > 60 and norm <= 70:
        print '{:|<70}'.format(str(i.table.name)+'\t') + \
            '\t ' + str(round(val, 2)) + ' MB'
    elif norm > 70 and norm <= 80:
        print '{:|<80}'.format(str(i.table.name)+'\t') + \
            '\t ' + str(round(val, 2)) + ' MB'
    elif norm > 80 and norm <= 90:
        print '{:|<90}'.format(str(i.table.name)+'\t') + \
            '\t ' + str(round(val, 2)) + ' MB'
    elif norm > 90 and norm <= 100:
        print '{:|<100}'.format(str(i.table.name)+'\t') + \
            '\t ' + str(round(val, 2)) + ' MB'
