...
for i in query_arr:
  if i == '*':
    print '\nPERFORMANCE WARNING: '
    print 'Do you need all the columns? You can improve the performance by only selecting the needed columns instead of doing a SELECT * statement'
  if i == 'WHERE':
    print '\nPERFORMANCE WARNING: '
    print 'Have you applied indexes on the columns that you use after the WHERE statement?'
  if i == 'the_geom' or i == 'the_geom_webmercator':
    print '\nPERFORMANCE WARNING: '
    print 'If the geometry is a polygon, have you simplified the geometries with the ST_Simplify() function?'