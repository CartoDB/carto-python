...
def test_get_visualizations(visualization_manager, user):
    ...
    :param visualization_manager: Fixture that provides a user manager to work with
    :param user: If valid, we'll check the number of returned visualizations exactly against the expected value
    """
    visualizations = visualization_manager.all()

    # If are testing against an enterprise account and have a valid user, we can easily know how many visualizations to expect
    if user is not None:
